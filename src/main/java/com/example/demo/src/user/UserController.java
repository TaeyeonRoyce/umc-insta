...
public class UserController {
	...
	public BaseResponse<PostUserRes> createUser(@RequestBody PostUserReq postUserReq) {
		...
		if (postUserReq.getEmail() == null) {
			...
			return new BaseResponse<>(POST_NAME_EMPTY);
		} else if (postUserReq.getPassword() == null) {
			return new BaseResponse<>(POST_PASSWORD_EMPTY);
		} else if (postUserReq.getNickname() == null) {
			return new BaseResponse<>(POST_NICKNAME_EMPTY);
		}
	...
	public BaseResponse<String> updateUserDetails(@PathVariable("userIdx") int userIdx, @RequestBody PatchDetailReq req) {
		try {
			...
			String date = dateFormat.format(req.getBirth());
			if (req.getBirth() != null && !isRegexDate(date)) {
				return new BaseResponse<>(PATCH_DETAIL_INVALID_URL);
			}
			req.setUserIdx(userIdx);
			userService.modifyUserDetail(req);
			String result = "회원 상세정보가 반영되었습니다.";
			return new BaseResponse<>(result);
		} catch (BaseException exception) {
			return new BaseResponse<>((exception.getStatus()));
		}
